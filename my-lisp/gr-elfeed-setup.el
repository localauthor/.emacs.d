;;; gr-elfeed-setup.el --- Setup elfeed     -*- lexical-binding: t; -*-
...
(use-package elfeed
  ...
  (defhydra hydra-elfeed (:hint nil
                                :color red)
    "
_l_: lit           _t_: today       _S_: scihub
_k_: knowledge     _w_: this week   _E_: add to ebib
_s_: starred       _A_: all         _D_: copy DOI
_a_: all unread    _U_: update      _O_: org list"
    ("U" (elfeed-update))
    ("O" (find-file (car rmh-elfeed-org-files)) :color blue)
...
(defun gr/elfeed-show-next (&optional n)
  (interactive "p")
  (elfeed-show-next n)
  (pop-to-buffer "*elfeed-entry*"))

(defun gr/elfeed-show-prev (&optional n)
  (interactive "p")
  (gr/elfeed-show-next (- (or n 1))))
...
(provide 'gr-elfeed-setup)
;;; gr-elfeed-setup.el ends here
